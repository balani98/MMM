...
@app.route('/api/build_pymc_model',methods=['POST'])
def build_the_pymc_model():
    try:
        ...
        result = { 
                    'body': {
                        'message':'PYMC model has been sent for building',
                        'task_id':res.id
                        },
                    'status':200
                }
...
@app.route('/api/pymc_model_results',methods=['GET'])
def pymc_model_results():
    try:
...
@app.route('/api/check_pymc_logs',methods=['GET'])
def check_pymc_model_status():
    try:
        log_file = 'pymc_info.log'
        pymc_model_status = check_pymc_logs(log_file)
        print(pymc_model_status)
        result = { 
                    'body': {
                        'pymc_model_status':pymc_model_status
                        },
                    'status':200
                }
        return result,200
...
def log_file_to_gcs():
    file_name =  get_latest_folder_with_files(BUCKET_NAME,file_path)
    remote_file_path = str(file_path)  + '/'+  str(file_name) + '/pymc_info.log'  
